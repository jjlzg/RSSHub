...
module.exports = async (ctx) => {
 ...
 if (ctx.params.limit === '25') {
      host = 'https://jiangxi.jxnews.com.cn/original';
      citytitle = '江西新闻';
    } else {
    ...
    const out = await Promise.all(
        list.map(async (item) => {
            ...
            //const itemUrl = 'http://m.baidu.com'
            ...
            const contents = capture('div.zwnr').html();
            //const time = capture.find('pubtime_baidu').text();
